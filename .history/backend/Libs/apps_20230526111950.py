...
class ProdutosCRUD:
    ...
    @handle_error
    def createProduto(self, codigo, codigo_completo, descricao, codigo_fornecedor, valor_custo, comissao, unidade, controle):
        # Primeiro, procurar pelo produto:
        result = self.session.query(mysql.Produtos).filter_by(codigo_completo).first()
        if result:
            ...
            objetoProduto = mysql.Produtos(
                id=None,
                codigo=int(codigo),
                codigo_completo=int(codigo_completo),
...
class CRUD:
    ...
    @handle_error
    def read(self, table, columnFilter, *args):
        if args:
            for i, arg in enumerate(args):
                result = self.session.query(table).filter(table.columnFilter == arg).first()
                if not result:
