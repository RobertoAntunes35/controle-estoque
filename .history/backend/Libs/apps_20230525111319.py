...
class FornecedoresCRUD:
    ...
    def readFornecedor(self, *args):
        '''For to choice a value specific, you need to one or more values to COLUMN codigo '''
        if args:
            for i, arg in args:
                result = self.session.query(mysql.Fornecedores).filter(mysql.Fornecedores.codigo == arg)
                print(result)
        else:
...
if __name__ == '__main__':
    ...
    crud_Fornecedor.readFornecedor('codigo', 11, 10)
