...
def busca_clientes():
    ...
    return jsonify({'updatedValueCliente': {
        ...
        'vendedorResponsavel':cliente.vendedor_responsavel,
        'dataEntrega':datetime.datetime.now()
    }})

@app.route('/adicionar_item_tabela', methods = ['GET', 'POST'])
def adicionar_item_tabela():
    codigoProduto = request.form['codigoProduto']
    nomeProduto = request.form['nomeProduto']
    quantidadeProduto = request.form['quantidadeProduto']
    unidadeProduto = request.form['unidadeProduto']
    valorProduto = request.form['valorProduto']
    valorTotal = round(request.form['valorTotal'], 3)
